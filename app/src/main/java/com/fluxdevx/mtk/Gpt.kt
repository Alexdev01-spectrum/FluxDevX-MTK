...
/** Standard GPT parser for a device-sector snapshot. */
...
object GptParser {
    ...
    fun parse(image: ByteArray): List<GptPartition> {
        ...
        require(entrySize >= ENTRY_SIZE && entrySize <= 4096) { "Unsupported GPT entry size: $entrySize" }
        val tableOffset = Math.multiplyExact(firstEntryLba, SECTOR.toLong()).toInt()
        val total = Math.multiplyExact(entryCount, entrySize)
        require(tableOffset >= 0 && tableOffset + total <= image.size) { "GPT entry table exceeds supplied image" }
    ...
    private fun uuidFromGpt(buffer: ByteBuffer): UUID {
        ...
        val hi = (a shl 32) or (b shl 16) or c
        val lo = buffer.long
        return UUID(hi, lo)
    }
}
